...
/* Touch-friendly slider — smooth dragging without CSS transition jitter */
const Slider = ({ label, value, onChange, min, max, suffix }) => {
    ...
    return (
        <div className="mb-6">
            ...
            >
                {/* Track background */}
                <div className="absolute left-0 right-0 h-2.5 bg-white/[0.06] rounded-full">
                    {/* Filled portion — NO transition for smooth dragging */}
                    <div
                        className="h-full bg-accent rounded-full"
                        style={{ width: `${percent}%` }}
                    />
                </div>
                {/* Thumb — NO transition during drag for instant response */}
                <div
                    className="absolute w-7 h-7 bg-white rounded-full border-2 border-accent"
                    style={{
...
const GrandirAnalytics = () => {
    ...
    return (
        <section id="analytics" className="relative py-16 md:py-20 overflow-hidden">
            ...
            <div className="px-5 md:px-8 max-w-3xl mx-auto relative z-10">
                {/* Section Header */}
                <div className="text-center mb-10">
                    <span className="text-xs font-semibold tracking-[0.3em] uppercase text-secondary mb-4 block">
                        Grandir Analytics
                    </span>
                    <h2 className="font-extrabold tracking-tighter leading-[1] font-display mb-4" style={{ fontSize: 'clamp(26px, 4vw, 40px)' }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                    {/* Sliders */}
                    <div>
                        <Slider label="Porcentaje de Operativa Manual" value={manualPercent} onChange={setManualPercent} min={1} max={100} suffix="%" />
                        <Slider label="Horas Empleadas / Semana" value={hoursPerWeek} onChange={setHoursPerWeek} min={1} max={80} suffix="h" />
                        <Slider label="Valor / Hora" value={hourlyValue} onChange={setHourlyValue} min={10} max={200} suffix="€" />
                    </div>
                    ...
                    <div className="bg-[#0A0A0A] border border-white/[0.06] rounded-2xl p-6 relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="mt-2 mb-4">
                                <span className="text-4xl md:text-5xl font-extrabold tracking-tighter text-white font-display">
                                    {calculations.annual} €
                                </span>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-2.5 mb-5">
                                <div className="bg-white/[0.04] border border-white/[0.06] rounded-xl p-3">
                                    <span className="text-[10px] font-semibold tracking-[0.15em] uppercase text-secondary block mb-1">Pérdida Mensual</span>
                                    <span className="text-lg font-bold text-white">{calculations.monthly} €</span>
                                </div>
                                <div className="bg-accent/10 border border-accent/20 rounded-xl p-3">
                                    <span className="text-[10px] font-semibold tracking-[0.15em] uppercase text-accent block mb-1">Eficiencia con Grandir</span>
                                    <span className="text-lg font-bold text-accent">+{calculations.efficiency}%</span>
                                </div>
                            </div>

                            <a href="https://cal.com/grupograndir/30min" target="_blank" rel="noopener noreferrer"
